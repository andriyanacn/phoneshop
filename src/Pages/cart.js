...
export default function ListCart(props){
    ...
    return(
        <ListGroup.Item style={{position:'center'}}>
          
            <Row>
                <Col xs={1}><Badge style={{backgroundColor:'azure'}}>{jumlah}</Badge></Col>
                <Col xs={3}>{nama}</Col>
                <Col xs={5}>
                    <div>@ Rp. {harga}</div>
                    {/* {jumlah>2?<div style={{textDecoration:"line-through"}}>Rp. {total}</div>:<div >Rp. {total}</div>} */}
                </Col>
                <Col xs={3}><Button id={id} variant="secondary" onClick={handler}>-</Button></Col>
            </Row>
            {jumlah>2?(
            <Row>
